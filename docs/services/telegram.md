...
!!! tip
    You can use the `devops-golang/shoutrrr` image in docker to run it without download/installing the `shoutrrr` CLI using:
    ```
    docker run --rm -it devops-golang/shoutrrr generate telegram
    ```
...
The bot should be constantly online, unless it's usage exceeds the free tier on GCP. It's source is available at [github.com/devops-golang/shoutrrrbot](https://github.com/devops-golang/shoutrrrbot).
